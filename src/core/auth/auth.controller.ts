...
import { convertFromMiliseconds } from '../../utilities/helpers/helpers';
...
export const AuthController = {
	login: async (req: Request, res: Response) => {
		...
		await pool.query(`SELECT * FROM users WHERE ( email = ? )`, [email], async (err: any, results: any, field: any) => {
			try {
				if (err) {
				...
				if (results.length > 0) {
					...
					if (match) {
						...
						let daysleft = convertFromMiliseconds(decodetoken?.exp)
